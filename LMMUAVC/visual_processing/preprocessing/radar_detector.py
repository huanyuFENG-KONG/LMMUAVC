...
def _prepare_frame_groups(
    ...
    for frame in radar_frames.values():
        if frame.size > 0:
            ...
            if frame.ndim == 1:
                # 一维数组，尝试reshape或跳过
                continue
            if frame.ndim == 2 and frame.shape[0] > 0:
    ...
    if point_dim is None:
        ...
        for frame in radar_frames.values():
            if frame.size > 0:
                if frame.ndim == 2:
                    point_dim = frame.shape[1] if frame.shape[0] > 0 else 3
                    break
        if point_dim is None:
            point_dim = 3  # 默认3维（xyz）
    ...
    for timestamp, raw_points in sorted_items:
        ...
        if raw_points.size == 0:
            # 空数组，创建空的二维数组
            filtered = np.empty((0, point_dim), dtype=np.float32)
        elif raw_points.ndim == 1:
            # 一维数组，跳过或reshape（如果可能）
            if raw_points.size > 0:
                # 尝试reshape为 (1, point_dim) 如果大小匹配
                if raw_points.size == point_dim:
                    filtered = raw_points.reshape(1, point_dim).astype(np.float32)
                else:
                    # 大小不匹配，跳过
                    filtered = np.empty((0, point_dim), dtype=np.float32)
            else:
...
class RadarCenterSequenceDetector:
        ...
        *,
        ...
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            ...
            if 'model_config' in checkpoint:
                ...
                # checkpoint中没有model_config，从state_dict推断配置
                inferred_input_size, inferred_hidden_size, inferred_num_layers = self._infer_model_config_from_state_dict(state_dict)
                inferred_dropout = dropout  # dropout无法从state_dict推断，使用默认值
                print(f"⚠ checkpoint中未找到model_config，从state_dict推断配置:")
                print(f"  input_size={inferred_input_size}, hidden_size={inferred_hidden_size}, num_layers={inferred_num_layers}, dropout={inferred_dropout}")
                hidden_size = inferred_hidden_size
                num_layers = inferred_num_layers
        else:
            ...
            inferred_input_size, inferred_hidden_size, inferred_num_layers = self._infer_model_config_from_state_dict(state_dict)
            inferred_dropout = dropout  # dropout无法从state_dict推断，使用默认值
            print(f"⚠ 加载直接state_dict格式，从state_dict推断配置:")
            print(f"  input_size={inferred_input_size}, hidden_size={inferred_hidden_size}, num_layers={inferred_num_layers}, dropout={inferred_dropout}")
            hidden_size = inferred_hidden_size
            num_layers = inferred_num_layers
    ...
    @staticmethod
    def _infer_model_config_from_state_dict(state_dict: dict) -> Tuple[int, int, int]:
        """
        从state_dict推断模型配置（input_size, hidden_size, num_layers）。
        ...
        """
        # 推断input_size：从lstm.weight_ih_l0的形状推断
        # lstm.weight_ih_l0的形状是 [4*hidden_size, input_size]
        if 'lstm.weight_ih_l0' in state_dict:
            weight_ih_l0 = state_dict['lstm.weight_ih_l0']
            input_size = weight_ih_l0.shape[1]
            # hidden_size = weight_ih_l0.shape[0] // 4 (LSTM有4个门)
        ...
        # 推断num_layers：检查是否有lstm.weight_ih_l1等
        num_layers = 1
        layer_idx = 1
        ...
        # 如果从fc.weight推断hidden_size（更准确）
        if 'fc.weight' in state_dict:
            fc_weight = state_dict['fc.weight']
            # fc.weight的形状是 [num_classes, hidden_size]
            inferred_hidden_size = fc_weight.shape[1]
            if inferred_hidden_size != hidden_size:
                # 使用fc.weight推断的值（更准确）
                hidden_size = inferred_hidden_size
                print(f"  从fc.weight推断hidden_size={hidden_size}（覆盖从lstm推断的值）")
